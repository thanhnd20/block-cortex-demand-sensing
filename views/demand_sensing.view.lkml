view: demand_sensing {
  ...
  SELECT
    ...
    FROM
      date) AS week
  FROM
    UNNEST(GENERATE_DATE_ARRAY(DATE_ADD('2022-02-14', INTERVAL -cast(@{years_of_past_data} as INT64) YEAR), DATE_ADD('2022-02-14', INTERVAL 13 WEEK))) AS date ),
  Grid AS (
  SELECT
...
ON
  ...
  dimension: moving_average {
    type: number
    sql:Case
        When CAST(${TABLE}.Date AS DATE) <= CAST ('2022-02-14' AS DATE)
  ...
  measure: thirteen_week_sale{
    ...
    sql:
    CASE
      WHEN CAST(${TABLE}.Date AS DATE) <= CAST ('2022-02-14' AS DATE) and DATE_DIFF(CAST ('2022-02-14' AS DATE), Cast(${TABLE}.Date as Date), Day)<=91
      THEN ${TABLE}.Sales
    ELSE
      NULL
    END;;
  ...
  measure: fifty_two_week_sale{
    ...
    sql:
    CASE
      WHEN ${TABLE}.RecordType='SalesOrders' and CAST(${TABLE}.Date AS DATE) <= CAST ('2022-02-14' AS DATE) and DATE_DIFF(CAST ('2022-02-14' AS DATE), Cast(${TABLE}.Date as Date), Day)<=366
      THEN ${TABLE}.Sales
    ELSE
      NULL
    END;;
  ...
  measure: thirteen_week_forecast{
    ...
    sql:
      CASE
        WHEN CAST(${TABLE}.Date AS DATE) > CAST ('2022-02-14' AS DATE) and DATE_DIFF(Cast(${TABLE}.Date as Date), CAST ('2022-02-14' AS DATE), Day)<=91
